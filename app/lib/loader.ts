...
export const loadAllPosts = async (WORDPRESS_URL: string, KV: KVNamespace, waitUntil: (promise: Promise<any>) => void): Promise<WP_REST_API_Posts> => {
  ...
  if (freshPosts) {
    ...
    const FetchingState = new KVStore<boolean>(POSTS,
      "FetchingState",
      { expirationTtl: 60 });
